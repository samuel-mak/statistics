...
```{r heteroscedasticity, include = F, echo = T}

data <- iris %>% dplyr::mutate(outcome = Petal.Length, predictor = Sepal.Length)

model <- lm(formula = outcome ~ predictor, data = data)

plot(model, which = 1)

png(file = "image/het_resid_vs_fitted.png", res = 100, width = 800, height = 500)
plot(model, which = 1)
dev.off()
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Heteroscedasticity**</p>
...
- **Visualisation**
  - ![](image/heteroscedasticity_visualisation.png){width=30%}
- **Mathematics**
...
variance (e.g. confidence intervals, test statistic, p-values etc.)
...
- **Assessing Homo/Heteroscedasticity**
  ...
  - **Plots**
    ...
    - **Residuals vs fitted plot**
      ...
      - **Visualisation**
        - ![](image/het_resid_vs_fitted.png){width=50%}
      - <details><summary>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Heteroscedasticity-Consistent Standard Error**</p>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**The Bootstrap (Efron & Tibshirani, 1993)**</p>
...
qqplot1.1 <- ggplot2::ggplot(data = iris, aes(sample = Sepal.Length)) + 
  qqplotr::stat_qq_point(size = 1, alpha = 0.6, detrend = TRUE) + 
  qqplotr::stat_qq_line(colour = "#008898", detrend = TRUE) + 
  qqplotr::stat_qq_band(fill = "#008898", alpha = 0.3, detrend = TRUE) + 
  labs(
    x = "Theoretical Quantile", 
    y = "Observed Quantile") +
  theme_minimal()
...
qqplot1
png(file = "image/qqplot1.1.png", res = 100, width = 800, height = 500)
qqplot1.1
dev.off()
```
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Robust Techniques**</p>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Robust Estimators**</p>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**M-estimators**</p>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Trimming**</p>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Winsorizing**</p>
...
- **Defining the threshold boundary **
  - **Percentage-based threshold boundary**
    - **Description**
      - The threshold boundary is determined by a percentage of the data
    **Example**
      - **The data**
      ...
      - **Winsorize**   
        - **5% Winsorization**
          ...
          - <mark>0</mark>, 0, 0, 1, 3, 3, 4, 4, 5, 8, 9, 9, 10, 11, 13, 17, 19, 19, 21, <mark>21</mark>
        - **10% Winsorization**
          ...
          - <mark>0, 0 </mark>, 0, 1, 3, 3, 4, 4, 5, 8, 9, 9, 10, 11, 13, 17, 19, 19, <mark>19, 19</mark>
        - **20% Winsorization**
          ...
          - <mark>3, 3, 3, 3 </mark>, 3, 3, 4, 4, 5, 8, 9, 9, 10, 11, 13, 17, <mark>17, 17, 17, 17</mark>
...
<p style = "margin-bottom: 0px; font-size: 20px; ">**Theil-Sen Estimator**</p>
