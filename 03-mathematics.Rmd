...
ggplot2::ggplot() + lims(x = c(0,20), y = c(0,20)) + 
  ...
  annotate(geom = "text", x = 7.25, y = 5.75, label = expression(~bold("c")), colour = "red") +
  annotate(geom = "text", x = 9, y = 4, label = expression(~bold(a)~"+"~bold(b)~"="~bold(c))) +
  theme_linedraw()
dev.off()
...
ggplot2::ggplot()  +
  ...
  geom_arrowsegment(aes(x = 3, y = 3, xend = 6, yend = 9), arrow_positions = 0.55, arrows = arrow(type = "closed", length = unit(0.1, "inches"))) +
    ...
    geom_arrowsegment(aes(x = 3, y = 3, xend = 12, yend = 8), arrow_positions = 0.55, arrows = arrow(type = "closed", length = unit(0.1, "inches")), arrow_fills = "#717171",  colour = "#717171") +
  ...
  annotate(geom = "text", x = 9.5, y = 3, label = expression(~bold(d))) +
...
**Resultant vector**
  ...
  - It is the vector that results from multiple vectors 
  - 
- **The triangle law for vector addition**
  - **Concept**
    - The resultant vector of 2 vectors is the vector that would form a triangle when it is linked up with the two vectors that have been successively put together 
  - **Mathematics**
...
- **Unit vectors parallel to the coordinate axes**
  - **Concept**
    ...
    - The vector $\mathbf{i} = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}$ is a unit vector that is parallel to the x-axis 
    - The vector $\mathbf{j} = \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix}$ is a unit vector and is parallel to the y-axis
    - The vector $\mathbf{k} = \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix}$ is a unit vector and is parallel to the z-axis 
  - A vector $\mathbf{v} = \begin{pmatrix} x \\ y \end{pmatrix}$ dan be written as $x\mathbf{i} + y\mathbf{j}$
