...
) |> dplyr::mutate(music = factor(music), food = factor(food)) |> rowid_to_column(var = "id")
...
lm(happiness ~ food*music, data = music_food_dat) |> summary()




car::Anova(lm(happiness ~ music*food, data = music_food_dat))

afex_object <- afex::aov_4(happiness ~ music + (1|id), data = music_food_dat)

music_food_dat$happiness |> mean()

afex::afex_plot(afex_object, "music") + scale_y_continuous(limits = c(-1, 18), breaks = seq(-1, 18, 1))





model.matrix(lm(happiness ~ music*food, data = music_food_dat))


lapply(X = music_food_dat$happiness, FUN = function(x){x + 1}) |> unlist()

music_food_dat[1] |> c()
music_food_dat


music_food_dat |> group_by(music, food) |> summarise(mean = mean(happiness))


gganova(formula = "happiness ~ music", data = music_food_dat, 
        geom_segment.group.mean = list(color = "red"))
...
```{r}
play_dat <- data.frame(x = c(1, 2, 3, 4, 5, 6, 7, 7, 7, 7, 8, 9, 10, 11, 11, 12, 15, 19, 20, 20, NA, NA, NA,22, 22, 23, NA)) |> rowid_to_column(var = "id")

play_dat
play_dat[!duplicated(play_dat$x),]



which(play_dat$x == "7")

filter(play_dat, is.na(x) | is.null(x)) |> nrow()

ccc_dat.2 <- ccc_dat[, c("Condition", "Gender", "SocialDesirability")] |> 
  mutate(
    SocialDesirability_2 = (SocialDesirability - mean(SocialDesirability))/sd(SocialDesirability),
    SocialDesirability_cat = ifelse(SocialDesirability_2 > -qnorm(p = 0.95, mean = 0), ifelse(SocialDesirability_2 > qnorm(p = 0.95, mean = 0), "High", "Mid"), "Low")
    ) 
