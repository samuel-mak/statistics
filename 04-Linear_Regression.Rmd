...
- **3-sample ANOVA as a Linear Model**
  - **Mathematics**
    - $y_{ik} = \beta_{0} + \beta_{1}x_1 + \beta_{2}x_2 + \epsilon_{ik}$
      - **Where**
        - $\beta_{0}$
          - $\beta_{0} = \mu_{reference}$
          - The mean of the reference group 
          - If there is a control group, the reference group is usually the control group
          - This represents the expected value of the outcome variable when the predictor variable is 0
        - $\beta_{1}$
          - $\beta_{1} = \mu_{1} - \mu_{0}$
          - Dummy variable 1
          - Dummy variable for category 1
          - A dummy variable that represents the effect of category 1 compared to the reference group as the difference between the mean of sample 1 and the mean of sample 0 (the reference group)
        - $x_{1}$
        ...
        - $\beta_{2}$
          ...
          - A dummy variable that represents the effect of category 2 compared to the reference group as the difference between the mean of sample 2 and the mean of sample 0 (the reference group)
        - $x_{2}$
...
- **Coding**
  ...
  - **Dummy Coding**
    - **Concept**
      - Compares each level to the intercept where the intercept is the mean of the reference group
  - **Simple Coding**
    - **Concept**   
      - Compares each level to the intercept where the intercept is the grand mean 
  - **Deviation coding**
    - **Concept**
...
- **Operationalising the Null Hypothesis of Two-Way ANOVA**
  - **General Concept**
    - There can be multiple hypotheses to be tested in a Two-Way ANOVA, but they are all based on the agreement/disagreement between 2 estimators of the variance of the population (the common variance $\sigma^2$)
    - 
